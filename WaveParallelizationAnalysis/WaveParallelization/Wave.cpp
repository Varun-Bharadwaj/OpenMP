...
int main()
{
	...
	cout << "Number of Sequential Operations: " << (nRows * nCols) << endl;
	...
	CopyArray(dataSeq, dataParallel, nRows, nCols);
	WaveParallelization(true, dataParallel, nRows, nCols);
	AverageTime = 0.0f;
	for (int i = 0; i < NUM_IT; i++)
	{
		CopyArray(dataSeq, dataParallel, nRows, nCols);
		start = std::chrono::system_clock::now();
		WaveParallelization(false,dataParallel, nRows, nCols);
		end = std::chrono::system_clock::now();
		std::chrono::duration<double> elasped_seconds = end - start;
		AverageTime += elasped_seconds.count();
	}
